...
```
./fly sp -t <YOUR_TARGET_NAME> configure -c pipeline.yml -p sink --load-vars-from=credentials.yml
...
```
./fly -t <YOUR_TARGET_NAME> unpause-pipeline -p sink
...
```
<YOUR_CONCOURSE_URL>/pipelines/sink
```
...
```
./fly -t <YOUR_TARGET_NAME> destroy-pipeline -p sink
```