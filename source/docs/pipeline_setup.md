...
```
./fly sp -t <YOUR_TARGET_NAME> configure -c pipeline.yml -p source --load-vars-from=credentials.yml
...
```
./fly -t <YOUR_TARGET_NAME> unpause-pipeline -p source
...
```
<YOUR_CONCOURSE_URL>/pipelines/source
```
...
```
./fly -t <YOUR_TARGET_NAME> destroy-pipeline -p source
```